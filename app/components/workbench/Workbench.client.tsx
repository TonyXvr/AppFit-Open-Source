...
import { ProductCoach } from './ProductCoach';
import { AiPersonas } from './AiPersonas';
// import { ImageGeneration } from './ImageGeneration'; // Commented out UI Previews component
import { VibeLearning } from './VibeLearning';
import { PromptBetterPanel } from './prompt-better';
...
export const Workbench = memo(
  ({ chatStarted, isStreaming, actionRunner, metadata, updateChatMestaData }: WorkspaceProps) => {
    ...
    useEffect(() => {
      ...
      // Check if we should open the Vibe Learning tab
      if (typeof window !== 'undefined') {
        const shouldOpenVibeLearning = localStorage.getItem('openVibeLearning');
        if (shouldOpenVibeLearning === 'true') {
          // Open the workbench and set the view to Vibe Learning
          workbenchStore.setShowWorkbench(true);
          setSelectedView('vibeLearning');

          // Clear the flag
          localStorage.removeItem('openVibeLearning');
        }

        // Check if we should open the image generation tab (commented out but keeping for reference)
        // const shouldOpenImageGeneration = localStorage.getItem('openImageGeneration');
        // if (shouldOpenImageGeneration === 'true') {
        //   // Open the workbench and set the view to image generation
        //   workbenchStore.setShowWorkbench(true);
        //   setSelectedView('imageGeneration');
        //
        //   // Clear the flag
        //   localStorage.removeItem('openImageGeneration');
        // }
      }
    }, [hasPreview]);
            ...
            <div className="absolute inset-0 px-2 lg:px-6">
              <div className="h-full flex flex-col bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor shadow-sm rounded-lg overflow-hidden">
                ...
                <div className="relative flex-1 overflow-hidden">
                  ...
                  <View initial={{ x: '100%' }} animate={{ x: selectedView === 'productCoach' ? '0%' : '100%' }}>
                    <ProductCoach chatStarted={chatStarted} isStreaming={isStreaming} />
                  </View>
                  <View initial={{ x: '100%' }} animate={{ x: selectedView === 'aiPersonas' ? '0%' : '100%' }}>
                    <AiPersonas chatStarted={chatStarted} isStreaming={isStreaming} />
                  </View>
                  <View initial={{ x: '100%' }} animate={{ x: selectedView === 'vibeLearning' ? '0%' : '100%' }}>
                    <VibeLearning chatStarted={chatStarted} isStreaming={isStreaming} />
                  </View>
                  {/* Commented out UI Previews tab */}
                  {/* <View initial={{ x: '100%' }} animate={{ x: selectedView === 'imageGeneration' ? '0%' : '100%' }}>
                    <ImageGeneration chatStarted={chatStarted} isStreaming={isStreaming} />
                  </View> */}
                </div>
              </div>
            </div>
