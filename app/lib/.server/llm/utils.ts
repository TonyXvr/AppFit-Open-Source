...
export function simplifyBoltActions(input: string): string {
  // First, clean any thinking mode content for clarity
  let cleaned = input;
  
  // Clean thinking prefixes
  cleaned = cleaned.replace(/^thinking:.*$/gm, '');
  
  // Clean thinking blocks
  cleaned = cleaned.replace(/<div class="__boltThought__">[\s\S]*?<\/div>/g, '');
  ...
  return cleaned.replace(regex, (_0, openingTag, _2, closingTag) => {
    return `${openingTag}\n          ...\n        ${closingTag}`;
  });
}
